    ...
    , changeHandler_(changeHandler)
{
	...
	protocol_choice_ = new wxChoice(&parent, nullID);
	lay.gbAdd(bag, protocol_choice_, lay.valigng);
	...
	row->Add(new wxStaticText(&parent, XRCID("ID_PORT_DESC"), _("&Port:")), lay.valign);
	...
	lay.gbNewRow(bag);
	lay.gbAdd(bag, new wxStaticText(&parent, XRCID("ID_JURISDICTION_DESC"), _("&Jurisdiction:")), lay.valign);

	auto jrow = lay.createFlex(0, 1);
	jrow->AddGrowableCol(0);
	lay.gbAdd(bag, jrow, lay.valigng);

	auto jurisdictions = new wxChoice(&parent, XRCID("ID_JURISDICTION"));
	jrow->Add(jurisdictions, lay.valigng);

	lay.gbNewRow(bag);
	encryption_desc_ = new wxStaticText(&parent, nullID, _("&Encryption:"));
	lay.gbAdd(bag, encryption_desc_, lay.valign);
	auto brow = new wxBoxSizer(wxHORIZONTAL);
	lay.gbAdd(bag, brow, lay.valigng);
	encryption_ = new wxChoice(&parent, nullID);
	brow->Add(encryption_, 1);
	...
	for (auto const& proto : CServer::GetDefaultProtocols()) {
		...
		if (entry.first != protocolGroups().cend()) {
			if (entry.second == entry.first->protocols.cbegin()) {
				mainProtocolListIndex_[proto] = protocol_choice_->Append(entry.first->name);
			}
			else {
			...
			mainProtocolListIndex_[proto] = protocol_choice_->Append(CServer::GetProtocolName(proto));
		}
	}
	if (protocol_choice_->GetCount()) {
		protocol_choice_->Select(0);
	}
	...
	protocol_choice_->Bind(wxEVT_CHOICE, [this](wxEvent const&) {
		auto p = GetProtocol();
		SetProtocol(p);
		if (changeHandler_) {
	...
	jurisdictions->Append(_("North America"));
	jurisdictions->Append(_("Europe Union"));
	jurisdictions->Append(_("FedRAMP"));
	jurisdictions->Select(0);
...
void GeneralSiteControls::SetSite(Site const& site)
{
	if (!site) {
		...
		xrc_call(parent_, "ID_JURISDICTION", &wxChoice::SetSelection, 0);
	}
	else {
		...
		if (site.server.GetProtocol() == CLOUDFLARE_R2) {
			...
			if (region.empty()) {
				...
				xrc_call(parent_, "ID_JURISDICTION", &wxChoice::SetSelection, 1);
			}
			else if (region == L"fedramp") {
				xrc_call(parent_, "ID_JURISDICTION", &wxChoice::SetSelection, 2);
			}
			else {
				xrc_call(parent_, "ID_JURISDICTION", &wxChoice::SetSelection, -1);
			}
...
void GeneralSiteControls::SetControlVisibility(ServerProtocol protocol, LogonType type)
{
	...
	encryption_desc_->Show(group.first != protocolGroups().cend());
	encryption_->Show(group.first != protocolGroups().cend());
	...
	xrc_call(parent_, "ID_HOST_DESC", &wxStaticText::Show, protocol != CLOUDFLARE_R2);
	xrc_call(parent_, "ID_HOST", &wxStaticText::Show, protocol != CLOUDFLARE_R2);
	xrc_call(parent_, "ID_PORT_DESC", &wxStaticText::Show, protocol != CLOUDFLARE_R2);
	xrc_call(parent_, "ID_PORT", &wxStaticText::Show, protocol != CLOUDFLARE_R2);
	xrc_call(parent_, "ID_JURISDICTION_DESC", &wxChoice::Show, protocol == CLOUDFLARE_R2);
	xrc_call(parent_, "ID_JURISDICTION", &wxChoice::Show, protocol == CLOUDFLARE_R2);
	...
	xrc_call(parent_, "ID_KEYFILE_DESC", &wxStaticText::Show, (protocol == SFTP || protocol == GOOGLE_CLOUD_SVC_ACC) && type == LogonType::key);
	xrc_call(parent_, "ID_KEYFILE", &wxTextCtrl::Show, (protocol == SFTP || protocol == GOOGLE_CLOUD_SVC_ACC) && type == LogonType::key);
	xrc_call(parent_, "ID_KEYFILE_BROWSE", &wxButton::Show, (protocol == SFTP || protocol == GOOGLE_CLOUD_SVC_ACC) && type == LogonType::key);
	...
	auto encSizer = encryption_->GetContainingSizer();
...
bool GeneralSiteControls::UpdateSite(Site & site, bool silent)
{
	...
	if (site.server.GetProtocol() == CLOUDFLARE_R2) {
		auto jurisdiction = xrc_call(parent_, "ID_JURISDICTION", &wxChoice::GetSelection);
		if (jurisdiction == wxNOT_FOUND || jurisdiction == 0) {
...
void GeneralSiteControls::SetProtocol(ServerProtocol protocol)
{
	...
	if (entry.first != protocolGroups().cend()) {
		...
		for (auto const& prot : entry.first->protocols) {
			...
			encryption_->AppendString(name);
		}
		encryption_->Show();
		encryption_desc_->Show();
		encryption_->SetSelection(entry.second - entry.first->protocols.cbegin());
	}
	else {
		encryption_->Hide();
		encryption_desc_->Hide();
	}

	auto const protoIt = mainProtocolListIndex_.find(protocol);
	if (protoIt != mainProtocolListIndex_.cend()) {
		protocol_choice_->SetSelection(protoIt->second);
	}
	else if (protocol != ServerProtocol::UNKNOWN) {
		auto const entry = findGroup(protocol);
		if (entry.first != protocolGroups().cend()) {
			mainProtocolListIndex_[protocol] = protocol_choice_->Append(entry.first->name);
			for (auto const& sub : entry.first->protocols) {
				mainProtocolListIndex_[sub.first] = mainProtocolListIndex_[protocol];
			}
		}
		else {
			mainProtocolListIndex_[protocol] = protocol_choice_->Append(CServer::GetProtocolName(protocol));
		}

		protocol_choice_->SetSelection(mainProtocolListIndex_[protocol]);
	}
	else {
		protocol_choice_->SetSelection(mainProtocolListIndex_[FTP]);
	}
...
{
	int const sel = protocol_choice_->GetSelection();
	...
	if (group.first != protocolGroups().cend()) {
		int encSel = encryption_->GetSelection();
...
void GeneralSiteControls::SetControlState()
{
	...
	protocol_choice_->Enable(!predefined_);
	encryption_->Enable(!predefined_);
...
bool GeneralSiteControls::ValidateGoogleAccountKeyFile(std::wstring_view filename)
{
	return true;
}
...
{
	wxSize const descSize = encryption_desc_->GetSize();
	wxSize const encSize = encryption_->GetSize();

	int dataWidth = std::max(encSize.GetWidth(), protocol_choice_->GetSize().GetWidth());

	auto generalSizer = static_cast<wxGridBagSizer*>(protocol_choice_->GetContainingSizer());
